...
import de.kiyan.ChangeSkin.nms.Skin1_5;
import de.kiyan.ChangeSkin.nms.Skin1_6;
...
public class SkinManager {
    ...
    private void updateSkin( Player player, String value, String signature ) {
        try {
            ...
            Constructor< ? > entitySpawn = getNMSClass( "PacketPlayOutNamedEntitySpawn" ).getConstructor( getNMSClass( "EntityHuman" ) );
            Constructor< ? > entityDestroy = getNMSClass( "PacketPlayOutEntityDestroy" ).getConstructor( int[].class  );

            // Apply the Enum & EntityPlayer to the instance
            Object playerConnection = entityPlayer.getClass().getField( "playerConnection" ).get( entityPlayer );

            for( Player target : Bukkit.getOnlinePlayers() ) {
                Object entityTarget = target.getClass().getMethod( "getHandle" ).invoke( target );
                Object targetConnection = entityTarget.getClass().getField( "playerConnection" ).get( entityTarget );
                Method sendPacket = targetConnection.getClass().getMethod( "sendPacket", getNMSClass( "Packet" ) );

                int[] id = new int[ 1 ];
                id[ 0 ] = (int) entityPlayer.getClass().getMethod( "getId" ).invoke( entityPlayer );

                sendPacket.invoke( targetConnection, playerInfo.newInstance( removeEnum, objArray ) );
                sendPacket.invoke( targetConnection, playerInfo.newInstance( addEnum, objArray ) );

                if( player != target ) {
                    sendPacket.invoke( targetConnection, entityDestroy.newInstance( id ) );
                    sendPacket.invoke( targetConnection, entitySpawn.newInstance( entityPlayer ) );
                }
            }
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    try {
                        ...
                        playerConnection.getClass().getMethod( "sendPacket", getNMSClass( "Packet" ) ).invoke( playerConnection, OutRespawn.newInstance( dimensionManager, dimensionKey, ( long ) seed, gameMode, gameMode, false, false, true ) );
                        player.teleport( loc );
    ...
    public void mineskinUpload( Player player, File skinFile ) {
        ...
        skinClient.generateUpload( skinFile, SkinOptions.create( skinFile.getName().replace( ".png", "" ), Model.DEFAULT, Visibility.PRIVATE ), new SkinCallback() {
            ...
            public void done( Skin skin ) {
                ...
                {
                    new Skin1_6().refreshPlayer( player, skinValues[ 0 ], skinValues[ 1 ] );
                } else if( version.contains( "1.15" ) )
                {
                    new Skin1_5().refreshPlayer( player, skinValues[ 0 ], skinValues[ 1 ]  );
                }
